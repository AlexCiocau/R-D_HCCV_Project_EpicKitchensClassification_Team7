...
class EpicKitchensDataset(Dataset):
    ...
    def __init__(self, path_to_data, num_frames, testing=False, transform=None):
        ...
        if not self.testing:
            print("Mode: TRAINING (Using custom_noun_train_80.csv)")
            # Make sure this CSV exists from your previous split step
            path_to_annotations = path_to_data + '/annotations/custom_noun_train_80.csv'
            TENSOR_OUTPUT_DIR_RAW = '$VSC_SCRATCH/x3d_train_tensors'
        else:
            print("Mode: VALIDATION (Using custom_noun_val_20.csv)")
            path_to_annotations = path_to_data + '/annotations/custom_noun_val_20.csv'
            TENSOR_OUTPUT_DIR_RAW = '$VSC_SCRATCH/x3d_train_tensors'
