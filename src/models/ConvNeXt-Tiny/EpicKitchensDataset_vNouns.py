...
class EpicKitchensDataset(Dataset):
    ...
    def __init__(self, path_to_data, num_frames, testing=False, transform=None):
        ...
        # --- CHANGED: Standard ImageNet Normalization ---
        # (Required for ResNet50 to work well with pre-trained weights)
        ...
        # --- CHANGED: Calculate Noun Classes ---
        if len(self.annotations) > 0:
            self.num_classes = self.annotations['noun_class'].max() + 1
        else:
            self.num_classes = 0
            
        print(f"Found {self.num_classes} unique noun classes.")
    ...
    # --------------------- NORMAL GETITEM (with transformations) -----------------------
    ...
    def __getitem__(self, idx):
        path, label = self.samples[idx]
        try:
            # Just load the raw tensor
            video_tensor = torch.load(path)
            
            # Ensure Float32 but DO NOT normalize or crop here anymore
            if video_tensor.dtype != torch.float32:
                 video_tensor = video_tensor.float()
                 
            # Return raw tensor (C, T, H, W)
            return video_tensor, label
